      ...
      <div v-else>
        ...
        <div v-else class="text-center py-10 text-gray-500">
          ...
          <p class="text-xs mt-4 text-gray-400">
            Se você cadastrou um evento e ele não aparece aqui, verifique se:
            <br />- O evento está ativo
            <br />- A data de término (end_date) é >= hoje
            <br />- As migrations foram executadas
          </p>
        </div>
      </div>
    </div>
  ...
  try {
    ...
    if (response.data) {
      if (Array.isArray(response.data)) {
        // Retorno direto (quando available=true)
